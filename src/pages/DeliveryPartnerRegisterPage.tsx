...
export function DeliveryPartnerRegisterPage() {
  ...
  const handleSubmit = async () => {
    setLoading(true);
    try {
      const response = await fetch('http://localhost:5000/api/delivery-partners/register', {
        method: 'POST',
        headers: {
      ...
      if (data.success) {
        alert('Registration submitted successfully! Please wait for admin approval. You will be notified once approved.');
              ...
              {s < 4 && (
                <div
                  className={`w-16 h-1 mx-2 ${
                    step > s ? 'bg-neutral-900 dark:bg-white' : 'bg-neutral-200 dark:bg-neutral-700'
                  }`}
                />
              )}
            </div>
          ...
          <div className="flex justify-between mt-8 pt-6 border-t border-neutral-200 dark:border-neutral-700">
            ...
            {step === 4 ? (
              <button
                ...
                disabled={loading}
                className="flex items-center gap-2 px-6 py-3 rounded-xl bg-neutral-900 text-white dark:bg-white dark:text-neutral-900 hover:bg-neutral-800 dark:hover:bg-neutral-100 disabled:opacity-50"
              >
              ...
              <button
                onClick={nextStep}
                className="flex items-center gap-2 px-6 py-3 rounded-xl bg-neutral-900 text-white dark:bg-white dark:text-neutral-900 hover:bg-neutral-800 dark:hover:bg-neutral-100"
              >
                Next
